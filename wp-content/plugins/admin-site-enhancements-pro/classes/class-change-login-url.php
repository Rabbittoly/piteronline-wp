...
class Change_Login_URL {
            ...
            if ( is_user_logged_in() ) {
                ...
                if ( array_key_exists( 'redirect_after_login', $options ) && $options['redirect_after_login'] ) {
                    ...
                    if ( isset( $user->roles ) && is_array( $user->roles ) ) {
                        $current_user_roles = $user->roles; // sort by value in descending order, so roles with custom redirection enabled comes first
                    }
                    
                    if ( 'single_url' == $redirect_after_login_type
                        && array_key_exists( 'redirect_after_login_for', $options ) 
                        && ! empty( $options['redirect_after_login_for'] ) 
                    ) {
                        ...
                        if ( isset( $redirect_after_login_for ) && ( count( $redirect_after_login_for ) > 0 ) ) {
                            ...
                            foreach ( $current_user_roles as $role ) {
                                if ( in_array( $role, $roles_for_custom_redirect ) ) {
                                    if ( isset( $_GET['action'] ) ) {
                                        ...
                                        if ( 'switch_to_user' == $_GET['action']
                                            || 'switch_to_olduser' == $_GET['action'] 
                                        ) {
                                            return; // This ensures user switching proceeds
                                        } else {
                            ...
                            if ( isset( $_GET['action'] ) && 
                                ( 'switch_to_user' == $_GET['action'] // User Switching plugin
                                || 'switch_to_olduser' == $_GET['action'] ) // User Switching plugin
                            ) {
                                return; // This ensures user switching proceeds
                            }
                        }
                    } else if ( 'separate_urls' == $redirect_after_login_type
                        && array_key_exists( 'redirect_after_login_for_separate_role', $options ) 
                        && ! empty( $options['redirect_after_login_for_separate_role'] ) 
                    ) {
                        // For the Pro version where each user role can have their own redirect URL
                        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                            $redirect_after_login_for_separate_slug_raw = isset( $options['redirect_after_login_for_separate_slug'] ) ? $options['redirect_after_login_for_separate_slug'] : ''; // array of role => slug
                            $redirect_after_login_for_separate_role = $options['redirect_after_login_for_separate_role']; // array of role => true/false
    ...
    public function redirect_on_default_login_urls() {
        ...
        $url_input = sanitize_text_field( $_SERVER['REQUEST_URI'] ); // e.g. /wp-admin/index.php?page=page-slug
        ...
        if ( ( isset( $_POST['log'] ) && isset( $_POST['pwd'] ) ) 
            ...
            if ( isset( $url_input_parts[1] ) 
                && 'wp-login.php' == $url_input_parts[1] 
                && empty( $_POST ) 
            ) {
            ...
            if ( isset( $url_input_parts[1] )
                && in_array( $url_input_parts[1], array( 'admin', 'wp-admin', 'login', 'wp-login', 'wp-login.php', 'login.php' ) )
                && ( ! isset( $url_input_parts[2] ) 
                    || ( isset( $url_input_parts[2] ) && empty( $url_input_parts[2] ) ) // e.g. /wp-admin/
                    || ( isset( $url_input_parts[2] ) && false !== strpos( $url_input_parts[2], '.php' ) ) // e.g. /wp-admin/profile.php
                )
            ) {
    ...
    public function redirect_to_custom_login_url_on_login_fail() {
        ...
        if ( isset( $asenha_limit_login ) && is_array( $asenha_limit_login ) && $asenha_limit_login['within_lockout_period'] ) {
            ...
            if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                ...
                //     if ( in_array( $request_url, array( $checkout_url, $myaccount_url ) ) ) {
                //         $should_redirect = false;
                //     }
                // }

                // Prevent redirection to wp-login.php when user failed logging in in a JetFormBuilder login form
                // if ( function_exists( 'is_plugin_active' ) && is_plugin_active( 'jetformbuilder/jet-form-builder.php' ) ) {
                //     if ( isset( $_POST['_jet_engine_booking_form_id'] ) && isset( $_POST['_jet_engine_refer'] ) ) {
                //         $should_redirect = false;
                //     }
                // }
