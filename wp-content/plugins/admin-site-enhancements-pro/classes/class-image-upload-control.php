...
class Image_Upload_Control {
    ...
    public function image_upload_handler( $upload ) {
        ...
        if ( in_array( $upload['type'], $applicable_mime_types ) ) {
            ...
            $mime_types_to_resize = array( 'image/jpeg', 'image/jpg', 'image/png', 'image/webp' );
            ...
            if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                ...
                $mime_types_to_convert = array( 'image/jpeg', 'image/jpg', 'image/png' );

                // Let's convert to WebP
                if ( $convert_to_webp  && in_array( $upload['type'], $mime_types_to_convert ) ) {
                    // Get file extension
                    if ( false !== strpos( $upload['file'], '.png' ) ) {
                        $file_extension = 'png';
                    }
                    if ( false !== strpos( $upload['file'], '.jpg' ) ) {
                        $file_extension = 'jpg';
                    }
                    if ( false !== strpos( $upload['file'], '.jpeg' ) ) {
                        $file_extension = 'jpeg';
                    }
                    ...
                    $webp_path      = $wp_uploads['path'] . '/' . $new_filename;
                    
                    // Let's convert to WebP based on the active editor
                    // which is shown in Tools >> Site Health >> Info >> Media Handling
                    $active_editor = _wp_image_editor_choose();

                    // For wp_image_editor_supports()
                    // $args = array(
                    ...
                    if ( 'WP_Image_Editor_GD' == $active_editor && function_exists( 'imagewebp' ) ) {
                        // Generate image object from PNG/JPG, then generate WebP from it
                        $this->gd_generate_webp( 
                            $upload['file'], 
                            $file_extension, 
                            $webp_path, 
                            $webp_conversion_quality 
                        );
                    } elseif ( 'WP_Image_Editor_Imagick' == $active_editor && class_exists( 'Imagick' ) ) {
                        if ( in_array( $file_extension, array( 'png', 'jpg', 'jpeg' ) ) ) {
                            // Image is a JPG / JPEG or non-transparent PNG. Let's convert with Imagick
                            $imagick = new Imagick();
                            $supported_formats = $imagick::queryFormats();
                            if ( in_array( 'WEBP', $supported_formats ) ) {
                                // Ref: https://www.jclabs.co.uk/generate-webp-images-in-php-using-and-gd-or-imagick/
                                $imagick->readImage( $upload['file'] );
                                $imagick->setImageFormat( 'webp' );
                                // $imagick->setFormat( 'webp' );
                                $imagick->setImageCompressionQuality( $webp_conversion_quality );
                                if ( 100 == $webp_conversion_quality ) {
                                    $imagick->setOption( 'webp:lossless', 'true' );
                                }
                                $imagick->writeImage( $wp_uploads['path'] . '/' . $new_filename );
                            } else {
                                // Imagick does not support WebP, let's try to use GD
                    ...
                    // elseif ( 'WP_Image_Editor_Imagick' == $active_editor && wp_image_editor_supports( $args ) ) {
                    //     $wp_image_editor = wp_get_image_editor( $upload['file'] );
                    //     if ( ! is_wp_error( $wp_image_editor ) ) {
                    //         $wp_image_editor->set_quality( $webp_conversion_quality );
                    //         $wp_image_editor->save( $wp_uploads['path'] . '/' . $new_filename, 'image/webp' );
                    //     }
                    // }

                    // maybe delete original BMP/PNG/JPG
                    $keep_original_image = isset( $options['keep_original_image'] ) ? $options['keep_original_image'] : false ;
