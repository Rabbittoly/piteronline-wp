...
class WooCartTrackingInit
{
    ...
    public function handleOrderCreated($order)
    {
        ...
        $order->add_meta_data('_fc_ab_cart_id', $abCart->id);

        //   $this->maybeHandleCartOrderChanged($order, $abCart);
    }
    ...
    public function maybeHandleCartOrderChanged(\WC_Order $order, $abCart = null)
    {
        ...
        if (!AbCartHelper::isWinOrderStatus($status)) {
    ...
    public function parseSmartCodes($code, $valueKey, $defaultValue, $subscriber)
    {
        ...
        switch ($valueKey) {
            ...
            case 'shipping_total':
                return $abCart->shipping ?: $defaultValue;
            case 'tax_total':
                return $abCart->tax ?: $defaultValue;
            ...
            case 'shipping_phone':
                ...
                $value = $abCart->getAddressProp($key, 'shippingAddress');
                if (!$value) {
                    $value = $abCart->getAddressProp($key, 'billingAddress');
                }
