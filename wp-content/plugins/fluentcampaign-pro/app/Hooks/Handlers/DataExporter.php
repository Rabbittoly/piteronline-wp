...
class DataExporter
{
    ...
    public function importEmailSequence()
    {
        ...
        foreach ($emails as $email) {
            ...
            if ($sequenceMail['design_template'] == 'visual_builder') {
                $email->_visual_builder_design = fluentcrm_update_campaign_meta(
                    $sequenceMail['id'],
                    '_visual_builder_design',
                    $email['_visual_builder_design']
                );
            }
        }
    ...
    public function exportEmailSequence()
    {
        ...
        foreach ($data['emails'] as $email) {
            if ($email['design_template'] == 'visual_builder') {
                $email->_visual_builder_design = fluentcrm_get_campaign_meta(
                    $email['id'],
                    '_visual_builder_design',
                    true
                );
    ...
    public function importRecurringCampaign()
    {
        ...
        wp_send_json([
            'message' => __('Recurring Email Campaigns has been successfully imported', 'fluent-campaign-pro'),
