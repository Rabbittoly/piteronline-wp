                    ...
                    foreach ($raw_field['choices'] as $raw_choice) {
                       ...
                       if(isset($raw_choice['image']))
                        ...
                        $field->options['choices'][] = $choice;
                                ...
                                if( isset( $raw_field['products'] ) ) {
                    $field->products = [];
                    foreach ( $raw_field['products'] as $raw_product ) {
                        if( ! isset( $raw_product['product_id'] ) ) continue;
                        $field->products[] = [
                            'slug'          => sanitize_text_field( $raw_product['slug'] ),
                            'label'         => sanitize_text_field( $raw_product['label'] ),
                            'product_id'    => intval( $raw_product['product_id'] ),
                            'selected'      => $raw_product['selected'] == 'true' ? true : false,
                        ];
                    }
                }
