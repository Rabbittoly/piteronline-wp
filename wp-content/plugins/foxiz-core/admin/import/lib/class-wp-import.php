...
class RB_WP_Import extends WP_Importer {
	...
	function import_start( $file ) {
		...
		if ( is_wp_error( $import_data ) ) {
			wp_send_json_error( esc_html__( 'Sorry, there has been an error.', 'foxiz-core' ), 400 );
...
<?php
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			Ruby_Importer::mark_as_imported( $id, 'category' );
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			Ruby_Importer::mark_as_imported( $id, 'tag' );
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			Ruby_Importer::mark_as_imported( $id, $term['term_taxonomy'] );
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					Ruby_Importer::mark_as_imported( $post_id );
			...
			if ( ! empty( $post['comments'] ) ) {
				...
				foreach ( $newcomments as $key => $comment ) {
					...
					if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
						...
						Ruby_Importer::mark_as_imported( $inserted_comments[ $key ] );
	...
	function process_menu_item( $item ) {
		...
		Ruby_Importer::mark_as_imported( $id );
