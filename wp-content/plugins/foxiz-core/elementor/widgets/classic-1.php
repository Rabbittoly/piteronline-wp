...
use function foxiz_get_classic_1;
use function foxiz_is_ruby_template;

class Classic_1 extends Widget_Base {

	public function get_name() {

		return 'foxiz-classic-1';
	}

	public function get_title() {

		return esc_html__( 'Foxiz - Classic', 'foxiz-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options'     => Options::columns_dropdown( [ 0, 1, 2, 3 ] ),
				'default'     => '0',
			]
		);
		$this->add_control(
		...
		$this->add_responsive_control(
			'content_width', [
				'label'       => esc_html__( 'Content Max Width', 'foxiz-core' ),
				'type'        => Controls_Manager::NUMBER,
				'description' => esc_html__( 'Input a max width value (in px) for the content of this layout.', 'foxiz-core' ),
				'selectors'   => [ '{{WRAPPER}} .p-wrap' => '--content-width: {{VALUE}}px;' ],
			]
		);
		$this->end_controls_section();
		$this->start_controls_section(
	...
	protected function render() {
		...
		if ( function_exists( 'foxiz_get_classic_1' ) ) {

			$settings         = $this->get_settings();
			$settings['uuid'] = 'uid_' . $this->get_id();
			echo foxiz_get_classic_1( $settings );
		}
	}
}