...
use function foxiz_get_list_flex;
use function foxiz_is_ruby_template;

class List_Flex extends Widget_Base {

	public function get_name() {

		return 'foxiz-list-flex';
	}

	public function get_title() {

		return esc_html__( 'Foxiz - Flex List', 'foxiz-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'flipped',
			[
				'label'        => esc_html__( 'Featured Flipped', 'foxiz-core' ),
				'type'         => Controls_Manager::SELECT,
				'description'  => esc_html__( 'Display the featured image flipped, first on the left side and then on the right side.', 'foxiz-core' ),
				'options'      => [
					'no'  => esc_html__( '- Default -', 'foxiz-core' ),
					'yes' => esc_html__( 'Flipped', 'foxiz-core' ),
				],
				'prefix_class' => 'flipped-',
				'default'      => 'no',
			]
		);
		$this->add_control(
	...
	protected function render() {
		...
		if ( function_exists( 'foxiz_get_list_flex' ) ) {
			...
			echo foxiz_get_list_flex( $settings );
		}
	}
}