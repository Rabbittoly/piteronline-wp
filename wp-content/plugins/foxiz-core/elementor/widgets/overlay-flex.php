...
use function foxiz_get_overlay_flex;
 ...
 */
class Overlay_Flex extends Widget_Base {

	public function get_name() {

		return 'foxiz-overlay-flex';
	}

	public function get_title() {

		return esc_html__( 'Foxiz - Flex Overlay', 'foxiz-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => esc_html__( 'Allow you to sort order elements to show such as category, title, meta...', 'foxiz-core' ),
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
			]
		);
		$this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'ai'          => [ 'active' => false ],
				'rows'        => 2,
				'description' => esc_html__( 'Input element keys to show, separate by comma. e.g. category, title, meta', 'foxiz-core' ),
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .meta-category a' => '--cat-highlight: {{VALUE}}',
				],
			]
		);
		$this->add_control(
	...
	protected function render() {
		...
		if ( function_exists( 'foxiz_get_overlay_flex' ) ) {
			...
			echo foxiz_get_overlay_flex( $settings );
		}
	}
}