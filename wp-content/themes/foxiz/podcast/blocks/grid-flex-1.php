...
if ( ! function_exists( 'foxiz_get_podcast_grid_flex_1' ) ) {
	 ...
	 * @return false|string
	 */
	function foxiz_get_podcast_grid_flex_1( $settings = array(), $_query = null ) {
		...
		if ( ! $_query->have_posts() ) {
			...
			foxiz_loop_podcast_grid_flex_1( $settings, $_query );
...
if ( ! function_exists( 'foxiz_loop_podcast_grid_flex_1' ) ) {
	/**
	 * @param  $settings
	 * @param $_query
	 */
	function foxiz_loop_podcast_grid_flex_1( $settings, $_query ) {
            ...
            <div class="post-carousel swiper-container pre-load" <?php foxiz_carousel_attrs( $settings ); ?>>
                <div class="swiper-wrapper">
					<?php while ( $_query->have_posts() ) :
						$_query->the_post();
						foxiz_podcast_grid_flex_1( $settings );
					endwhile;
					?>
                </div>
