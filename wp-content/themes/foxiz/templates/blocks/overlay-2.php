...
if ( ! function_exists( 'foxiz_get_overlay_2' ) ) {
	 ...
	 * @return false|string
	 */
	function foxiz_get_overlay_2( $settings = [], $_query = null ) {
		...
		$settings['classes'] = 'block-overlay block-overlay-2';
		...
		if ( empty( $settings['crop_size'] ) ) {
			$settings['crop_size'] = foxiz_get_option( 'grid_1_crop_size' );
		}
		...
		if ( ! $_query->have_posts() ) {
			...
			foxiz_loop_overlay_2( $settings, $_query );
...
if ( ! function_exists( 'foxiz_loop_overlay_2' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function foxiz_loop_overlay_2( $settings, $_query ) {
